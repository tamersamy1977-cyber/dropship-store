          ...
          <div className="flex-1 overflow-y-auto p-4">
            {items.length === 0 ? (
              ...
              <ul className="space-y-4">
                {items.map((item) => (
                  <li key={item.product.id} className="flex gap-4 bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-semibold text-blue-600 mt-1">{formatPrice(item.product.price)}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <button
                          onClick={() => updateQuantity(item.product.id, item.quantity - 1)}
                          className="w-7 h-7 rounded border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-200 text-sm"
                        >
                          -
                        </button>
          ...
          {items.length > 0 && (
            <div className="border-t border-gray-200 p-4 space-y-3">
              ...
              <Link
                href="/checkout"
                onClick={closeCart}
                className="block w-full bg-blue-600 text-white text-center font-medium py-3 rounded-lg hover:bg-blue-700 transition-colors"
              >
                Proceed to Checkout
              </Link>
            </div>
